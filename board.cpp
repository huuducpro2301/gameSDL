...
void board::render_score(SDL_Renderer *renderer)
{
    SDL_Rect rect={706,90,80,80};
    render_text(renderer,to_string(score[0]%100+(score[0]/100)*10-sub[0]),rect,30);
    //render_number(renderer,score[0]%100+(score[0]/100)*10-sub[0],rect,30);
    rect={520,520,80,80};
    render_text(renderer,to_string(score[1]%100+(score[1]/100)*10-sub[1]),rect,30);
    //render_number(renderer,score[1]%100+(score[1]/100)*10-sub[1],rect,30);
    rect={590,63,80,80};
    int k=score[0]%100+(score[0]/100)*10-sub[0];
    render_image(renderer,stone_texture[k],rect);
...
void board::render_board(SDL_Renderer *renderer)
{
    ...
    SDL_Rect rect;
    rect={0,0,1080,640};
    render_image(renderer,map_texture,rect);
    for (int i=0;i<12;i++)
    {
        cell[i].render_tile(renderer);
    }
    ...
    if (flag==1)
    {
        render_image(renderer,flag_texture,{600,500,40,40});
    }
    else render_image(renderer,flag_texture,{510,73,40,40});
    SDL_RenderPresent(renderer);
}
void board::move_next(SDL_Renderer *renderer,int start,int side)
{
    int cur=start;int num=cell[start].num;cell[start].num=0;
    render_board(renderer);
        render_image(renderer,hand2_texture,cell[cur].rect);
        SDL_RenderPresent(renderer);
        SDL_Delay(100);
    for (int i=1;i<=num;i++)
    {
        move_image(renderer,hand_texture,cell[cur].rect,cell[nnext[cur]].rect,1);
        cur=nnext[cur];
        cell[cur].num++;
        SDL_Delay(100);
    ...
    cur=nnext[cur];
    if (cur==0||cur==11) {render_board(renderer); return;}
    if (cell[cur].num>0)
    {
        move_image(renderer,hand_texture,cell[nback[cur]].rect,cell[cur].rect,1);
        move_next(renderer,cur,side);
    }
    else
        while (cell[cur].num==0)
        {
            ...
            {
                cell[cur].num=0;
                move_image(renderer,hand_texture,cell[cur].rect,rect_score,1);
                render_board(renderer);
...
void board::move_back(SDL_Renderer *renderer,int start,int side)
{
    int cur=start;int num=cell[start].num;cell[start].num=0;
    render_board(renderer);
    render_image(renderer,hand2_texture,cell[cur].rect);
    ...
    for (int i=1;i<=num;i++)
    {
        move_image(renderer,hand_texture,cell[cur].rect,cell[nback[cur]].rect,1);
        cur=nback[cur];
        cell[cur].num++;
    ...
    if (cell[cur].num>0)
    {
        move_image(renderer,hand_texture,cell[nnext[cur]].rect,cell[cur].rect,1);
        move_back(renderer,cur,side);
    }
    else
        while (cell[cur].num==0)
        {
...
void board::reset_none_side(SDL_Renderer *renderer,int side)
{
    for (int i=1;i<=5;i++)
...
void board::endgame(SDL_Renderer *renderer)
{
    SDL_Delay(300);
    for (int i=1;i<=10;i++)
    {
    ...
    render_image(renderer,blank_texture,{350,278,400,100});
    render_image(renderer,menu_tile_texture,{350,278,400,100});
    if (a1==a2) render_text(renderer,"draw",{400,300,400,100},50);
    if (a1<a2) render_text(renderer,name[0]+" win",{400,300,400,100},40);
    if (a1>a2) render_text(renderer,name[1]+" win",{400,300,400,100},40);
...
void board::move_image(SDL_Renderer *renderer,SDL_Texture *texture,SDL_Rect org,SDL_Rect deter,int time)
{
    int num=10;
    int s=abs(org.x-deter.x)+abs(org.y-deter.y);
    num+=(s/200)*5;
    render_image(renderer,texture,org);
    SDL_RenderPresent(renderer);
   // SDL_Delay(delay);
    for (int i=num;i>=1;i--)
    {
